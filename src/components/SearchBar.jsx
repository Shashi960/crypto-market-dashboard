...
export default function SearchBar({ search, setSearch, nextUpdateProgress, onRefresh }) {
  return (
      ...
      <div className="flex items-center gap-4 w-full md:w-auto">
        <div className="flex items-center gap-3 bg-white dark:bg-[#1A1F2E] rounded-full px-4 py-2 border border-gray-300 dark:border-gray-800">
          ...
          <div className="w-16 h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 transition-all duration-1000 ease-linear" style={{ width: `${nextUpdateProgress}%` }} />
          </div>
        </div>
        <button
          onClick={onRefresh}
